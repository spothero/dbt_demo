        with z as (
          ...
          WHERE (((CONVERT_TIMEZONE('UTC', spothero_city.timezone, transient_inventory_status.starts ) >= ((DATEADD(week,-4, DATE_TRUNC('week', DATE_TRUNC('day',GETDATE())) ))) AND CONVERT_TIMEZONE('UTC', spothero_city.timezone, transient_inventory_status.starts ) < ((DATEADD(week,4, DATEADD(week,-4, DATE_TRUNC('week', DATE_TRUNC('day',GETDATE())) ) )))))) AND (NOT COALESCE(transient_inventory_status.is_cnp , FALSE)) and transient_available > 0 and (NOT is_unavailable OR (unavailable_reason in (''))) AND DATE_PART(hour, CONVERT_TIMEZONE('UTC', spothero_city.timezone, transient_inventory_status.starts ))::integer >= 6 AND DATE_PART(hour, CONVERT_TIMEZONE('UTC', spothero_city.timezone, transient_inventory_status.starts ))::integer < 19 AND MOD(EXTRACT(DOW FROM CONVERT_TIMEZONE('UTC', spothero_city.timezone, transient_inventory_status.starts ))::integer - 1 + 7, 7) NOT IN (5,6) AND NOT COALESCE(parking_spot.actual_spot_id IS NOT NULL , FALSE)
          ...
          SELECT
                ...
                ,SUM(avg_stalls_available) as est_p4w_avg_commuter_stalls_available
                ,SUM(min_stalls_available) as est_p4w_min_commuter_stalls_available
          FROM z
          WHERE uninterrupted_hours >= 12
          GROUP BY 1