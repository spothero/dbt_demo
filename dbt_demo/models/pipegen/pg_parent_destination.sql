...
LEFT JOIN {{ source('sh_public','event') }} event_sg_fan ON summarized_event_table.seatgeek_id = event_sg_fan.seatgeek_id AND event_sg_fan.seatgeek_id IS NOT NULL
LEFT JOIN (
  SELECT
    event.event_id
    ,seo_url
  ...
  WHERE (count_ids = 1 OR count_ids IS NULL) AND event.seatgeek_id IS NULL
...
LEFT JOIN (
  ...
  FROM {{ source('sh_public','event') }} event
  LEFT JOIN title_starts_summary ON title_starts_summary.title_starts = event.title||event.starts
  WHERE (count_ids > 1) AND event.seatgeek_id IS NULL
