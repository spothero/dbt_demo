...
LEFT JOIN 
  {{ source('sh_public', 'parking_spot') }} origin_parking_spot 
  ON origin_parking_spot.parking_spot_id = origin_opportunity.spot_id_c 
  AND NOT origin_opportunity.is_deleted
CROSS JOIN {{ source('sfdc', 'opportunity') }} comp_opportunity
LEFT JOIN 
  {{ source('sh_public', 'parking_spot') }} comp_parking_spot 
  ON comp_parking_spot.parking_spot_id = comp_opportunity.spot_id_c 
  AND NOT comp_opportunity.is_deleted
WHERE 
  (ACOS(SIN(RADIANS(origin_opportunity.lat_long_latitude_s)) 
    * SIN(RADIANS(comp_opportunity.lat_long_latitude_s)) 
    + COS(RADIANS(origin_opportunity.lat_long_latitude_s)) 
    * COS(RADIANS(comp_opportunity.lat_long_latitude_s)) 
    * COS(RADIANS(comp_opportunity.lat_long_longitude_s 
      - origin_opportunity.lat_long_longitude_s))) * 6371 
  <= 1.60934 
  OR origin_opportunity.id = comp_opportunity.id)
